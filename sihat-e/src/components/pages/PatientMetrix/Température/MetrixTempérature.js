...
import { logout, createTemperature, fetchTemperatureInfos, editTemperature, deleteTemperature } from "../../../../actions";
...
class MetrixTemperature extends Component {
  constructor() {
    super();
    this.state = {
      show: false,
  ...
  createTemperature = (formValues) => {
    this.props.createTemperature(formValues);}
  ...
  renderList() {  
    if (!this.props.temperatureData.temperature_reducer.temperature.historique) {
        <Loader />
    }
    else if  (this.props.temperatureData.temperature_reducer.temperature.historique) {
     
      // const id = this.props.PoidsData.tailles_reducer.tailles.historique.id;
    return this.props.temperatureData.temperature_reducer.temperature.historique
    .map(thetemperatureData => {
      return (
        <div key={thetemperatureData.id} >
 ...
 <div class="login-box-seperator" id="login-box-seperator-left"></div>
                    <div id='taillemetricyourmetric'>
                        <div>
                            ...
                            <p>
                            au : {thetemperatureData.date}
                            </p>
                            </strong>
 
          {/* {this.upd = (formValues) => {this.props.editTaille(theglucoseData.id, formValues)}} */}
        {/* Creat */}
        { this.editTemperature = (formValues) => this.props.editTemperature(thetemperatureData.id, formValues)} 
        ...
        <Field
                ...
                label="Modifier votre temperature  :"
                placeholder="Votre temperature "
                type="text"
                span=''
        /> 
                        Cet indice date de: {thetemperatureData.date}
        </ModalUpdate>
                        </div>
      ...
      { this.del = () => this.props.deleteTemperature(thetemperatureData.id)} 
  ...
  render() {
    return this.props.temperatureData.temperature_reducer.loading 
    ? 
    (              
      <Loader />
      ) 
    : 
    this.props.temperatureData.temperature_reducer.error 
    ? 
    ( <h2>{this.props.temperatureData.temperature_reducer.error}</h2> )
    : 
    (
...
<div id="modal" className="d-xl-flex justify-content-xl-center align-items-xl-center">
          <div id="formCardContainer">
            <div>
              ...
              <Modal submit={this.props.handleSubmit(this.createTemperature)} modalTitle={this.state.modalTitle} show={this.state.show} handleClose={this.hideModalCreate}>
                              {/* children */}
          <strong><p>Température :</p></strong>
            <Field
                ...
                placeholder="Ajouter votre Température"
                type="text"
                span=''
            /> 
                        <strong><p>Date de cette température :</p></strong>
          <Field
                className="form-control"
                name="date"
                component={this.renderInput}
                label="La date de votre température :"
                type="date"
          /> 
...
const mapDispatchToProps = (dispatch, formValues, id) => {
  return {
    ...
    createTemperature : (formValues) => dispatch(createTemperature(formValues)),
    editTemperature : (formValues, id) => dispatch(editTemperature(formValues, id)),
    deleteTemperature : (id) => dispatch(deleteTemperature(id)),
  };
};

MetrixTemperature = connect(
    mapStateToProps,
    mapDispatchToProps
)(MetrixTemperature);

export default reduxForm({
    form: 'MetrixTemperatureHistoryandAdd', // a unique name for this form
